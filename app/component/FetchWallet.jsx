...
const FetchWallet = ({ data, length, postPerPage }) => {
  return (
    <div className={`p-3 ${length != 0 ? null : "pn:max-md:hidden"}`}>
      ...
      <div
        className={`${
          length > postPerPage ? "max-h-[250px]" : "h-[47vh] max-h-full"
        }  -z-10 bg-white dark:bg-[#0D0D0D] border no-scrollbar rounded-b-2xl overflow-y-scroll no-scrollbar`}
      >
        <Table>
                ...
                {data?.map((p, i) => (
                  <TableRow key={i} className="h-[70px]">
                    ...
                    <TableCell className="flex justify-center items-center text-center">
                      <div
                        className={`text-center rounded-lg max-w-[135px] font-medium px-4 py-2 ${
                          p?.status === "pending"
